...
class Helper
{
    ...
    public static function imageUpload(UploadedFile $mainFile, string $uploadPath, ?int $reqWidth = null, ?int $reqHeight = null): array
    {
        try {
    ...
    //     if (file_exists($path)) {
    //         // Escape for values with spaces or special characters
    //         $value = preg_match('/\s/', $value) ? "\"$value\"" : $value;

    //         // Replace existing env key
    //         file_put_contents(
    //             $path,
    //             preg_replace(
    //                 "/^{$key}=.*/m",
