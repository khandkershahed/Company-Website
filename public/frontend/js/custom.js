...
$(document).ready(function () {
    ...
    $(".email-validate").on("input", function () {
        ...
        if (email === "") {
            ...
            const missingParts = [];
            if (!email.includes("@")) missingParts.push("@");
            if (!email.includes(".")) missingParts.push(".");
            if (!email.includes("com")) missingParts.push("com");
...
// reCAPTCHA safe submit
function onSubmit(token) {
    if ($("#recaptcha-form").length) {
        document.getElementById("recaptcha-form").submit();
    }
    if ($(".get_quote_frm").length) {
        $(".get_quote_frm").submit();
    }
}

// Software Hardware Tab Slider
document.addEventListener("DOMContentLoaded", function () {
...
// TinyMCE
$(document).ready(function () {
    if (typeof tinymce !== "undefined" && $("#common").length) {
        tinymce.init({
            selector: "#common",
            plugins:
                "anchor autolink charmap codesample emoticons image link lists media searchreplace table visualblocks wordcount",
...
$(document).ready(function () {
    ...
    $(".qty-btn-plus").on("click", function () {
        let v = parseInt($inputQty.val(), 10);
        if (!isNaN(v)) $inputQty.val(v + 1);
    });

    $(".qty-btn-minus").on("click", function () {
        let v = parseInt($inputQty.val(), 10);
        if (!isNaN(v) && v > 1) $inputQty.val(v - 1);
    });
});
...
// Slick + Select2
$(document).ready(function () {
    if ($.fn.slick && $(".SlickCarousel").length) {
        $(".SlickCarousel").slick({
            autoplay: true,
            autoplaySpeed: 10000,
            speed: 1600,
    ...
    if ($.fn.select2) {
        $(".select2, .select_country").select2();
    }
});
