    ...
    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="lost_rfq_{{ $lost_rfq->id }}" role="tabpanel">
        <div class="card shadow-none">
                ...
                <div>
                    ...
                    <div>
                        <select class="form-select form-select-sm lostRFQ" id="tabSelector">
                            ...
                            <option value="delete_rfq_{{ $lost_rfq->id }}">Delete</option>
                        </select>
                    </div>

                    @push('scripts')
                        <script>
                            document.querySelectorAll('.lostRFQ').forEach(function(selectElement) {
                                selectElement.addEventListener('change', function() {
                                    const selected = this.value;
                                    if (selected === 'delete_rfq') {
                                        const confirmed = confirm('Are you sure you want to delete this RFQ?');
                                        if (confirmed) {
                                            window.location.href = "{{ route('admin.rfq.destroy', [$lost_rfq->id]) }}";
                                        } else {
                                            this.value = 'track_tab_{{ $lost_rfq->id }}'; // Reset selection if not confirmed
                                        }
                                    }
                                });
                            });
            ...
            <div>
                ...
                <div id="message_container_{{ $lost_rfq->id }}" class="container-hidden_{{ $lost_rfq->id }}">
                    <div class="row">
                        <div class="col-lg-12">
                                ...
                                <div class="card-header pe-5" id="kt_drawer_chat_messenger_header">
                                    <div class="card-title">
                                        <div class="d-flex justify-content-center flex-column me-3">
                                            <a href="#"
                                                class="fs-4 fw-bold text-gray-900 text-hover-primary me-1 mb-2 lh-1">{{ $lost_rfq->name }}</a>
