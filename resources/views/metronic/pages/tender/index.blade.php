<x-admin-app-layout :title="'Tender Information'">
                ...
                <div class="mb-5 col-6 col-sm-6 col-md-4 col-xl col-12 custom-col">
                    <div class="shadow-none card card-flush card-rounded h-100">
                        ...
                        <div class="pt-2 card-body d-flex flex-column justify-content-end pe-0">
                            <span class="pt-1 mb-2 tender-p-para text-black">{{ $card['label'] }}</span>
    ...
    @push('scripts')
        {{-- <script>
            $(document).on('click', '.showTender', function() {
                let id = $(this).data('id');

                $("#tenderShowModal").modal('show');
                $("#tender-details-body").html(`
                ...
                $.ajax({
                    url: "/admin/tender/show/" + id, // using ID directly
                    type: "GET",
                    success: function(res) {
                        $("#tender-details-body").html(res.html);
                    }
                });
            });
        </script> --}}
